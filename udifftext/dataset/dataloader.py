...
class AnywordDataset(data.Dataset):
    def __init__(self, cfgs, datype):
        super().__init__()
        with open(os.path.join(cfgs.data_root, cfgs.json_name), "r") as fp:
            data = json.load(fp)
        self.type = datype
        self.data_root = os.path.join(cfgs.data_root, "images")
        self.character = cfgs.character if hasattr(cfgs, 'character') else "abcdefghijklmnopqrstuvwxyz"
        self.word_len = cfgs.word_len if hasattr(cfgs, 'word_len') else (1, 20)
        ...
        for img_info in data["data_list"]:
            image_path = os.path.join(self.data_root, img_info["img_name"])
            for annotation in img_info["annotations"]:
                text = annotation["text"]
                bbox = np.array(annotation["polygon"])

                if len(text) < self.word_len[0]:
                    continue

                if len(text) > self.word_len[1]:
                    text = text[:self.word_len[1]]
        ...
        print(f"Loaded {self.length} valid items.")
